...
const roll = (sides) => {  
  return Math.floor(Math.random() * sides+1); // add 1 to sides because since this is Math.floor, sides is exclusive
};

/// rolls a random value between -sides and sides
const deviate = (sides) => {
  return (Math.floor(Math.random() * (sides*2)+1)) - sides;
};
...
const runFight = (request, response) => {
  ...
  // get each fighter
  Fighter.FighterModel.findByNameId(req.body.fighterName1, req.body.fighterId1, (err1, fighter1) => {
    if(err1) {
      console.log(err1);
      return res.status(400).json({ error: 'An error occurred' });
    }
    Fighter.FighterModel.findByNameId(req.body.fighterName2, req.body.fighterId2, (err2, fighter2) => {
      if(err2) {
        console.log(err2);
        return res.status(400).json({ error: 'An error occurred' });
      }
      ...
      // this is so if something messes up, it won't subtract health without finishing the battle and giving a result
      ...
      while(fighter1.tempHealth > 0 && fighter2.tempHealth > 0) {
        ...
        // but the order will always go: higher roll, lower roll, rest of the rounds from higher roll
        ...
        for(let i = 0; i < turns-1 /*cause we already ran a round*/; i++) {
          battleRound(attacker, defender, battleLog);
...
const battleRound = (attacker, defender, battleLog) => {
  // roll damage for attacker
  let damage = attacker.damage + deviate(5); // damage can deviate by 5 on either side (15 damage can hit from 10 to 20)
  ...
  battleLog.push(`${defender.name} took ${damage} damage and has ${Math.max(defender.tempHealth, 0)} health remaining`);
};

const isDead = (fighter) => {
  if(fighter.tempHealth < 1)
    return true;
  else
    return false;
}
...
const runArmor = (dmg, arm) => {
  ...
  // cause if they figure out how to get negative armor, they could have like a 2,400% chance to block damage lol
  ...
  // note that it's 1/m and 1/a to get the inverse of each one. I just did this so the numbers make more sense to me
  // 1/m makes it so the number the graph approaches is 'm'. and 1/a is so that as 'a' increases, the initial slope of the graph is higher
  // So right now it is approaching a 95% chance to block damage. I think a 95% chance at infinity armor is pretty good. 
  // That means it won't get into 99%s or anything like that. Deal 100 damage and only ~1 goes through. That would suck lmfao
  ...
  // so at 15, the max they can set from the beginning, chance to block every point of damage is about 42%
  // chance at 50 is 68%
